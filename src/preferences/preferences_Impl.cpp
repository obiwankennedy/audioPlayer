    ...
    : QDialog(parent, f), m_tabs(tabs)
{
    ...
    filterscb << cb_mp3 << cb_ogg << cb_mid << cb_mpc << cb_wav << cb_flac << cb_ape << cb_alf << cb_aac << cb_mp4
              << cb_m4a << cb_wma << cb_vqf << cb_vqe << cb_vql << cb_au;
...
void Preferences_Impl_Dialog::doc_browserchooser()
{
    ...
    *fileName= QFileDialog::getOpenFileName(
        this, tr("Play List Generator"), mypreference->getUri_documentation(), tr("Documentation Profile (*.adp)"));
    if(!fileName->isEmpty())
    {
