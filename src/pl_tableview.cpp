...
PL_TableView::PL_TableView(QWidget* p) : QTableView(p), m_model(NULL)
{
...
void PL_TableView::onModify()
{
    ...
    if(dialog->exec())
    {
        AmendTag* myamendtag= new AmendTag(dialog->artistEdit->text(), dialog->cbartist->isChecked(),
            dialog->TitleEdit->text(), dialog->cbtitre->isChecked(), dialog->albumEdit->text(),
            dialog->cbalbum->isChecked(), dialog->genreCombobox->currentIndex(), dialog->cbgenre->isChecked(),
            dialog->uriTextedit->text(), dialog->uriChanged(), dialog->commetEdit->text(),
            dialog->cbcomment->isChecked(), dialog->lineEdit->text(), dialog->cbYear->isChecked(), true);
