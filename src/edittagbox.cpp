...
#include <QFileDialog>
#include <QTextStream>
EditTagBox::EditTagBox(QWidget* parent, QList<QModelIndex>* mylist, int i) : QDialog(parent)
{
    setupUi(this);
    myselection= mylist;
...
void EditTagBox::changeUri()
{
    ...
    *fileName= QFileDialog::getOpenFileName(this, tr("Play List Generator"), mydir.absolutePath(),
        tr("Supported files (*.mp3)\nAll kind of file files (*.*)"));
    if(!fileName->isEmpty())
    {
        urichanged= true;
