...
PL_MediaFile* MediaFacade::buildaMedia(QString& txt, ItemDecorator*& key, SongFields* fields)
{
    ...
    else if((txt.endsWith(".ogg", Qt::CaseInsensitive)) || ((txt.endsWith(".m4a", Qt::CaseInsensitive)))
            || ((txt.endsWith(".mpc", Qt::CaseInsensitive))) || ((txt.endsWith(".oga", Qt::CaseInsensitive)))
            || ((txt.endsWith(".flac", Qt::CaseInsensitive))))
    {
        // out << ++i << " " <<txt <<endl;
        tmp= oggfactory->buildMedia(txt, fields);
        key= new OggDecorator;
