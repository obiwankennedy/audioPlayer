...
PL_Mp3File::PL_Mp3File(QString& uri, SongFields* fields) : PL_AudioFile(uri), fields(NULL)
{
    mode= 0;
    linked= false;
    this->uri= &uri;
...
{
    out << (fields->Title.simplified()) << (fields->Artist.simplified()) << (fields->duration) << (fields->Year)
        << (fields->genre) << (fields->album.simplified()) << (fields->Comment.simplified()) << (fields->Bitrate)
