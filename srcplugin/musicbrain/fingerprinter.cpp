...
Fingerprinter::Fingerprinter(const QString& filename)
    : filename_(filename), event_loop_(new QEventLoop), convert_element_(NULL)
{
}
...
GstElement* Fingerprinter::CreateElement(const QString& factory_name, GstElement* bin)
{
    GstElement* ret= gst_element_factory_make(factory_name.toAscii().constData(), factory_name.toAscii().constData());
