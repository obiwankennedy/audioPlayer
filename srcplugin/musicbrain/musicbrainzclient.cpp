...
NetworkTimeouts::NetworkTimeouts(int timeout_msec, QObject* parent) : timeout_msec_(timeout_msec) {}
...
MusicBrainzClient::MusicBrainzClient(QObject* parent)
    : QObject(parent), timeouts_(new NetworkTimeouts(kDefaultTimeout, this))
{
}
...
void MusicBrainzClient::Start(int id, const QString& puid)
{
    ...
    parameters << Param("type", "xml") << Param("puid", puid);
