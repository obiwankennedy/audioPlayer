...
LyricsFinder::LyricsFinder()
{
    ...
    QObject::connect(http, SIGNAL(finished(QNetworkReply*)), this, SLOT(readLyrics(QNetworkReply*)));

    QObject::connect(http2, SIGNAL(finished(QNetworkReply*)), this, SLOT(readLyrics(QNetworkReply*)));
...
void LyricsFinder::search()
{
    if(fields != NULL)
    {
        ...
        QString url= QString("http://www.leoslyrics.com/%1/%2-lyrics/")
                         .arg(artist.replace(' ', "-"))
                         .arg(title.replace(' ', "-"));
...
void LyricsFinder::readLyrics(QNetworkReply* p)
{
    ...
    *a= p->readAll();
    ...
    if(m_lyrics->contains(
           "<div ondragstart=\"return false;\" onselectstart=\"return false;\" oncontextmenu=\"return false;\">")
        && m_lyrics->contains("</div>"))
    {
        QRegExp exp("<div ondragstart=\"return false;\" onselectstart=\"return false;\" oncontextmenu=\"return "
                    "false;\">(.*)</div>");
        exp.setMinimal(true);
